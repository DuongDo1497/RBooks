...
<div id="address-container">
    <div class="container">
        ...
        <div class="h6 p-3">{{ trans('home.Chọn địa chỉ giao hàng có sẵn bên dưới:') }}</div>
        <div class="pl-3 address-question">{{ trans('home.Bạn muốn giao hàng đến địa chỉ khác?') }} <a href="{{ route('create-address') }}">{{ trans('home.Thêm địa chỉ giao hàng mới') }}</a>
        </div>
        @if (session('alert'))
        ...
        <div class="customer-address-default mt-4">
            <form action="{{ route('checkout-process') }}" role='form' method="post">
                ...
                <div class="btn-address">
                    ...
                    <button type="button" class="btn btn-default" onclick="window.location.href='{{ route('edit-address', ['id' => $address->id ]) }}'" {{ $address->default == 0 ? "hidden = 'true'" : '' }}>{{ trans('home.Sửa') }}</button>
        ...
        <div class="customer-address-nodefault mt-4 mb-3">
            <form action="{{ route('checkout-process') }}" role='form' method="post">
                ...
                <div class="btn-address">
                    ...
                    <button type="button" class="btn btn-default" onclick="window.location.href='{{ route('edit-address', ['id' => $address->id ]) }}'" {{ $address->default != 0 ? "hidden = 'true'" : '' }}>{{ trans('home.Sửa') }}</button>
                    <button type="button" class="btn btn-default" onclick="window.location.href='{{ route('delete-address', ['id' => $address->id ]) }}'" {{ $address->default != 0 ? "hidden = 'true'" : '' }}><i class="fa fa-trash" aria-hidden="true"></i></button>
                </div>
            </form>
        </div>
        @endif
