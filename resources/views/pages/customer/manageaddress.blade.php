          ...
          <div class="col-xxl-8">
            <div class="card account-body manager-address">
              <div class="card-header">
                <h4 class="profile-address">{{ trans('home.Địa chỉ của tôi') }}</h4>
                <button class="btn new-address" type="submit" data-bs-toggle="modal" data-bs-target="#modalAddAddress">+
                  {{ trans('home.THÊM ĐỊA CHỈ MỚI') }} </button>
              </div>
              <div class="card-body">
                @if (session('thongbao'))
                      ...
                      <table style="width:100%">
                        <tr>
                          ...
                          <td>{{ $address->fullname }}<span>
                              ...
                              <span class="btn-edit"><a href="#"data-bs-toggle="modal"
                                  data-bs-target="#modalEditAddress"> Chỉnh sửa</a></span>
                            </span></td>
                        </tr>
                        <tr>
                          ...
                          <td>
                            {{ $address->phone }}
                            <span class="btn btn-default btn-set-default btn-set-default__disabled">Thiết lập mặc
                              định</span>
                      ...
                      <table style="width:100%">
                        <tr>
                          ...
                          <td>{{ $address->fullname }}<span>
                              ...
                              <span class="btn-edit btn-edit__notdefault"><a href="#"data-bs-toggle="modal"
                                  data-bs-target="#modalEditAddress"> Chỉnh sửa</a> <a
                                  href="#"data-bs-toggle="modal" data-bs-target="#modalDeleteAddress">Xóa</a></span>
                            </span></td>
                        </tr>
                        <tr>
                          <th width="25%">Số điện thoại</th>
                          <td>
                            {{ $address->phone }}
                            <span class="btn btn-default btn-set-default btn-set-default__enabled">Thiết lập mặc
                              định</span>
      ...
      <div class="modal-editaddress">
        <form method="post" class="form-editaddress" id="form">
          ...
          <div class="float-default">
            ...
            <button onclick="functionOficeEditAddress()" style="float: right;" class="office-editaddress"
              type="button">Văn phòng</button>
            <button onclick="functionHomeEditAddress()" style="float: right;" class="home-editaddress"
              type="button">Nhà riêng</button>
          </div>
          <hr style="opacity: 0.1;">
      ...
      <div class="modal-addaddress">
        <form method="post" class="form-addaddress" id="form">
          <div class="addaddress">
            <h5>{{ trans('home.Thêm địa chỉ mới') }}</h5>
          </div>
          <hr style="opacity: 0.1;">
          <div class ="form-group-edit">
            <div class = "form-group-editaddress">
              <div class="text-name">
                <label for="name">{{ trans('home.Họ và tên') }}</label>
                <input autocomplete="off" type="text" id="name" required>
              </div>
            </div>
            <div class = "form-group-phone">
              <div class="text-phone">
                <label for="phone">{{ trans('home.Số điện thoại') }}</label>
                <input autocomplete="off" type="text" id="phone" required pattern="(\+84|0)\d{9}">
              </div>
            </div>
          </div>
          <div class="form-group-address">
            <div class="text-address">
              <div class="form-group-city">
                <div class="text-address-city">
                  <label for="city">{{ trans('home.Tỉnh/Thành phố') }}</label>
                  <select autocomplete="off" type="text" id="city" required></select>
                </div>
              </div>
              <div class="form-group-district">
                <div class="text-address-district">
                  <label for="district">{{ trans('home.Quận/Huyện') }}</label>
                  <select autocomplete="off" type="text" id="district" required></select>
                </div>
              </div>
              <div class="form-group-wards">
                <div class="text-address-wards">
                  <label for="wards">{{ trans('home.Phường/Xã') }}</label>
                  <select autocomplete="off" type="text" id="wards" required></select>
                </div>
              </div>
            </div>
            <div class = "form-group-specifically">
              <div class="text-specifically">
                <label for="specifically">{{ trans('home.Địa chỉ cụ thể') }}</label>
                <input autocomplete="off" type="text" id="specifically" required>
              </div>
            </div>
          </div>
          <div class="float-default">
            <label>
              <input type="checkbox" checked="checked" name="remember"> {{ trans('home.Đặt làm địa chỉ mặc định') }}
            </label>
            <button onclick="functionOficeAddress()" style="float: right;" class="office-address"
              type="button">{{ trans('home.Văn phòng') }}</button>
            <button onclick="functionHomeAddress()" style="float: right;" class="home-address"
              type="button">{{ trans('home.Nhà riêng') }}</button>
          </div>
          <hr style="opacity: 0.1;">
          <div class="btn-backsave">
            <button class="btn-back"><i class="fa-solid fa-arrow-rotate-left"></i>
              {{ trans('home.Quay lại') }}</button>
            <button class="btn-save"><i class="fa-solid fa-bookmark"></i> {{ trans('home.Lưu') }}</button>
          </div>
        </form>
      </div>
    </div>
...
<!-- Xóa -->
<div class="modal fade" id="modalDeleteAddress" data-bs-keyboard="false" tabindex="-1"
