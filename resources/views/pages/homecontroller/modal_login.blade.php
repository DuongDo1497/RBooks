      ...
      <div class="modal-header-login">
        <form action="{{ route('login') }}" method="post" class="form-horizontal" id="formLogin">
          ...
          <div class="d-flex justify-content-between align-items-center">
            <h3 class="form-login">{{ trans('home.Đăng nhập') }}</h3>
          </div>
          <div class="form-group {{ $errors->has('name') || $errors->has('password') ? ' has-error' : '' }}">
            <div class="form-group-email-password row">


              <input class="form-control" id="confirmEmail" type="text" name="name"
                placeholder="{{ trans('home.Tên tài khoản/Email') }}" value="{{ old('name') }}"
                onkeyup="manageBtn(this)">

              @if ($errors->has('name') || $errors->has('password') || $errors->has('email'))
                <span class="form-message-email-password">
                  <strong class="help-block">{{ trans('home.Sai tài khoản hoặc email hoặc mật khẩu') }} </strong>
                </span>
              @endif
            ...
            <div class="form-group-email-password row ">
              ...
              <input id="confirmPasswordLogin" type="password" name="password"
                placeholder="{{ trans('home.Mật khẩu') }}" class="form-control" onkeyup="manageBtn(this)">


              <span style="cursor:pointer ;" class="absolute-password"><i
                  class="fas fa-eye icon-eye-loginPwd eye-login "></i></span>
              <span style="cursor:pointer ;" class="absolute-password"><i
                  class="fas fa-eye-slash icon-eye-loginPwd eye-slash-login"></i></span>
              <span class="form-message-email-password"></span>
            </div>
            ...
            <div class="form-group row p-9">
              <div class="float-right">
                <a href="{{ route('password.reset') }}"
                  class="text-danger-Forgotpassword">{{ trans('home.Quên mật khẩu?') }} </a>
                <a href="#" data-bs-toggle="modal" data-bs-target="#modalRegister"
                  class="text-danger-register">{{ trans('home.Đăng ký') }}</a>
              </div>
            </div>
            <button class="btn btn-login" id="btnLogin" type="submit" disabled>{{ trans('home.Đăng nhập') }}</button>
