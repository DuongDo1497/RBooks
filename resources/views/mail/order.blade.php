	    	...
	    	<article>
	    		<div>
	    			...
	    			<div style="text-align: justify;">
	    				Đơn hàng #{{ $order->id }} của quý khách đã được đặt thành công vào lúc {{ date("H:i:s - d/m/Y", strtotime($order->created_at)) }}. Rbooks sẽ tiến hành giao hàng cho đơn vị vận chuyển và sẽ gửi đến quý khách trong khoảng từ 1 - 7 ngày.
	    			</div>
	    		</div>
	    		...
	    		<div>
	    			<div class="info payment">
	    				...
	    				<div>
	    					...
	    					<div>{{ $order->customer()->first()->email }}</div>
	    					@if($order->billingAddress->district == NULL & $order->billingAddress->city == NULL)
	    					<div>
	    						{{
	    			...
	    			<div class="info address">
	    				...
	    				<div>
	    					<div>{{ $order->billingAddress->fullname }}</div>
	    					<div>{{ $order->customer()->first()->email }}</div>
	    					@if($order->deliveryAddress->district == NULL & $order->deliveryAddress->city == NULL)
	    					<div>
	    						{{
	    		...
	    		<div>
	    			<div>
	    				...
	    				@endif
	    				<div style="padding-bottom: 10px;"><b>Thời gian giao hàng dự kiến:</b> dự kiến giao hàng {{ Carbon\Carbon::create()->addDays(7)->format('d/m/Y') }}
	    				</div>
